...
class Pix2PixModel(base_model.BaseModel):
    ...
    def __init__(self, opt, _isTrain=False):
        ...
        if self.mode == 'Ours_Bilinear':
            ...
            new_model = torch.nn.parallel.DataParallel(
                new_model.to(device), device_ids=range(torch.cuda.device_count()))
    ...
    def forward(self):
        ...
        # run first network
        self.input_images = autograd.Variable(self.input.to(device), requires_grad=False)
        human_mask = 1.0 - autograd.Variable(
            self.targets['env_mask'].to(device), requires_grad=False).unsqueeze(1)
        keypoints_img = autograd.Variable(
            self.targets['keypoints_img'].to(device), requires_grad=False).unsqueeze(1)
        ...
        if self.num_input == 7:
            input_log_depth = autograd.Variable(
                self.targets['input_log_depth'].to(device),
                requires_grad=False).unsqueeze(1)
            input_confidence = autograd.Variable(
                self.targets['input_confidence'].to(device),
                requires_grad=False).unsqueeze(1)
    ...
    def evlaute_M_error(self, input_, targets, n_iter, write_to_summary):
        ...
        input_imgs = autograd.Variable(input_.to(device), requires_grad=False)
        # stack inputs
        human_mask = 1.0 - autograd.Variable(
            targets['env_mask'].to(device), requires_grad=False).unsqueeze(1)
        keypoints_img = autograd.Variable(
            targets['keypoints_img'].to(device), requires_grad=False).unsqueeze(1)

        if self.num_input == 7:
            input_log_depth = autograd.Variable(
                targets['input_log_depth'].to(device), requires_grad=False).unsqueeze(1)
            input_confidence = autograd.Variable(
                targets['input_confidence'].to(device), requires_grad=False).unsqueeze(1)
    ...
    def eval_save_tum_img(self, input_, targets, save_path):
        input_imgs = autograd.Variable(input_.to(device))
        human_mask = 1.0 - autograd.Variable(
            targets['env_mask'].to(device), requires_grad=False).unsqueeze(1)
        keypoints_img = autograd.Variable(
            targets['keypoints_img'].to(device), requires_grad=False).unsqueeze(1)
    ...
    def run_and_save_videos_prediction(self, input_, targets, save_path):
        ...
        mvs_depth = autograd.Variable(targets['mvs_depth'].to(device), requires_grad=False)
        input_depth = autograd.Variable(
            targets['input_depth'].to(device), requires_grad=False)
